...
async function ensureDayPattern(inst, days) {
  const daysKey = [...days].sort((a, b) => WEEK_ORDER.indexOf(a) - WEEK_ORDER.indexOf(b)).join('-');
  let dp = await DayPattern.findOne({ institutionId: inst, daysKey });
...
async function seedTeaching(institution, owner, teacherHash) {
  ...
  for (const p of plan) {
    for (const [instr, dp, ts] of p.batches) {
      const name = `${instr.slice(0, 3).toUpperCase()}-${dp.daysKey.toUpperCase()}-${ts.startTime}`;
