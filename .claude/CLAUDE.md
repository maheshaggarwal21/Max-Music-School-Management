...
> files. **The canonical docs are trustworthy as of `39ac221`.** **PENDING user decision (flagged, NOT
> removed — the AskUserQuestion errored): 3 deletion candidates** — `max music screenshots/` (7.2 MB
> untracked legacy source, captured in `feature-inventory/legacy-maxmusic-screenshot-inventory.md`),
> `GSTACK_README.md` (44 KB tracked gstack readme, unreferenced, dup of `GSTACK.md`), and the
> `.claude/documentation/` memory trio (`MEMORY.md`+`project_maxmusic.md`+`reference_gstack.md`, redundant
> in-repo index). Re-ask before removing. Memory: `project_render_vercel_deploy`.
