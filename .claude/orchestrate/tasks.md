...
**Full E2E `/qa` IN PROGRESS (2026-06-11):** feature inventory of all 4 panels written (`documentation/feature-inventory/`); both P2 bugs FIXED (BUG-01 `daysKey` + live-DB migration `ca0b244`; BUG-02 `useId()` `85e2b47`); **operator panel phase COMPLETE — 6 live-mode contract bugs (ISSUE-002..007)**; **admin panel phase COMPLETE — 4 more (ISSUE-008..011)** + all admin workflows + white-label clean; **teacher panel phase COMPLETE — 3 more fixed + browser re-verified (ISSUE-013 holiday "Invalid Date NaN", ISSUE-014 nested-anchor roster, ISSUE-001 BlurFade ref warning on all panels = closes BUG-04)** + attendance/Socket.io/KPI/holidays/profile all verified + teacher console now fully clean; **ISSUE-012 CLOSED (not reproduced)**. (See `documentation/qa-e2e-2026-06-11.md` + AUDIT.md §7.) Box fits only ONE Next dev panel + API at a time (7.5 GB RAM) — QA proceeds panel-by-panel. **REMAINING: student panel (booted on :3003), then /qa close-out.**
...
**Next action (2026-06-11, teacher phase done):** finish E2E — **student panel** (:3003, booted: login by mobile 8888888888/Student@123, dashboard, classes, timetable, payments pending-contract empty state, profile edit/guardian fields, white-label sweep) → final QA report + /qa skill close-out. Then `/ship` (currently HELD) + VPS deploy (L11–L13). ISSUE-001 (BlurFade) FIXED `41a8634`; ISSUE-012 CLOSED (not reproduced). **NOTE — extra teacher cred for QA:** QA Guitar Teacher mobile `7777777777` / `Teacher@123` (displayId 100003) owns the one batch + student; use it for attendance/Socket.io/KPI (the owner teacher 100001 has no batches). **Before starting the stack: kill ALL listeners on ports 4000 + 3000–3003 first** (`netstat -ano | grep :PORT | grep -i listening` → `taskkill //F //PID`).
