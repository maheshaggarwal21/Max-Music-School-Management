...
**☁ CLOUD DEPLOY LIVE (2026-06-13):** first PUBLIC deploy — API on **Render** (free), 4 panels on **Vercel** (separate projects), same Atlas DB. All 4 logins browser-verified end-to-end (`/me`+dashboard 200, white-label clean). Blocker+fix = **`COOKIE_SAME_SITE=none`** on Render (cross-domain cookies: onrender.com↔vercel.app are different domains → browser drops any non-`SameSite=None;Secure` cookie; login 200 but no session). Also: `CORS_ORIGINS`=all 4 vercel URLs; Render Build Cmd=`npm install` only; Atlas IP `0.0.0.0/0`; Vercel needs Linux swc/oxide/lightningcss binaries (`ac78ceb`). Render free tier sleeps ~15min → ~30–60s cold start. god OTP live=`88990011` (works w/o MSG91). Guide: `DEPLOY.md §0`. Client doc: `Desktop/MaxMusic-Client-Testing-Guide.{docx,pdf}` (`scripts/make-client-testing-guide.py`). **VPS (L10–L13) still the production target.**
...
| P8-07 | Free-tier cloud deploy (Render API + Vercel x4 + Atlas) | DONE | 2026-06-13: PUBLIC demo LIVE; all 4 logins browser-verified. Fix = `COOKIE_SAME_SITE=none` (cross-domain cookies) + `CORS_ORIGINS` x4 + Render build=`npm install` + Atlas `0.0.0.0/0` + Linux native binaries (`ac78ceb`). Cold-start ~30-60s (free tier sleeps). god OTP live `88990011`. Guide: `DEPLOY.md` section 0. Client testing guide generated (docx+pdf). VPS L10-L13 still the prod target |
